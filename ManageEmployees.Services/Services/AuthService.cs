...
namespace ManageEmployees.Services.Services
{
    public class AuthService : IAuthService
    {
        ...
        {
            var tokenToRemove = await _refreshTokenRepository.GetByUserIdAsync(userId);

            if (tokenToRemove is null) return false;

            await _refreshTokenRepository.DeleteAsync(tokenToRemove.Id);
            return true;
        }
        ...
        private async Task<bool> ValidateRefreshToken(string userId, string refreshToken)
        {
            ...
            var dbRefresh = await _refreshTokenRepository.GetByUserIdAsync(userId);
            ...
            await _refreshTokenRepository.DeleteAsync(dbRefresh.Id);
            return true;
        }
    }
}
